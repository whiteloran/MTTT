...
​魔法少女的施法都与这两种法术力相联系。具体对每种法术力都有*增长*、*无关*、*消耗*三种形式。

| 关联     | 秩序消耗 | 秩序无关 | 秩序增加 |
| -------- | -------- | -------- | -------- |
| 歪曲消耗 | 常规法术        | 歪曲法术   | 调谐法术  |
| 歪曲无关 | 秩序法术  | 不存在   | 反秩序法术        |
| 歪曲增加 | 调谐法术  | 反歪曲法术   | 逆转法术      |
...
在战斗场景开始时，每位魔法少女的法术力池都是未展开状态，也就是说，*可用法术力上限*为0。
通过活化速度，在每回合开始时，魔法少女的*可用法术力上限*会增加相当于**活化速度**的数量。（活化速度对秩序法术力和歪曲法术力同时起效。）
通常状态下，大部分魔法少女不具备**活化速度**这一条目，她们使用**展开法力**这一动作来提高*可用法术力上限*（见战斗章节）。
一些少见的装备和灵魂宝石符文能够提供**魔力共鸣**，可以根据在场的*长河单位*的数量来活化法术力池。
在**魔力共鸣**之外，还有一些其它方法加快活化速度，这里不多赘述。
关于活化的具体应用，请看战斗场景一章。
...
和活化一样，法术池也仅仅在战斗场景中才会涉及到。魔法少女在每场战斗中能够使用的法术是有上限的。
在*每次游戏*开始时，魔法少女需要选择等同于其\[魔法属性值(10-30)+可能的其它调整值(最高可达到40)\]数量的法术，作为她的法术池。这些法术必须是她已经学会的。
魔法少女在一场战斗中只能使用其法术池内的法术。在法术池中，每个同名法术（即同一个法术）可以被选择不超过2(最高可达到4)次。
在法术池中的每个法术在使用一次后（被多次选择的同一法术可用次数相当于被选次数），其法术模型就会*陷入歪曲*，魔法少女无法再次使用这个法术直到该场战斗结束。
在每次战斗前，魔法少女会刷新她的法术池，让每个选定的法术回到未使用的状态。
直到下一次游戏开始，魔法少女重新选定她的法术池。


### 歪曲池：

所有魔法少女从法术池中施放法术，在施放时点后都会进入歪曲池。


#### 放逐池：

被放逐的法术不存在法术池或歪曲池中的任意一池，将会进入放逐池。通常没有什么法术可以影响到放逐池内的其它法术。
